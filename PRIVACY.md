...
1. **Provide an `api_url`** to the optional LLM-assisted image / Word
   ...
   endpoint you choose.
2. **First-run model downloads** by EasyOCR. These may fetch OCR model weights on first use and cache them under `~\.EasyOCR\model` by default. To run fully offline after first launch, pre-cache the models or preserve the cached directory.

## Third-party dependencies that may phone home

| Package | Behaviour | How to disable |
|---|---|---|
| `EasyOCR` | Downloads OCR model weights on first use | Pre-cache models by warming EasyOCR once online or bundle the cache |

The default code path does not use Hugging Face / transformers. No additional offline-only environment variables are required for the local OCR path.
...
4. (Optional) Delete `~\.EasyOCR\model` to remove cached OCR weights.
